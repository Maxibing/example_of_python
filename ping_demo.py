...
def request_ping(data_Sequence):
    '''
    创建request ping报文
    :param data_Sequence:
    :return icmp_packet:
    ...
    imcp_packet = struct.pack('>BBHHHd', data_type, data_code, data_checksum,
                              data_ID, data_Sequence, payload_body)
    icmp_chesksum = checksum(imcp_packet)  #获取校验和
    #  把校验和传入，再次打包
    imcp_packet = struct.pack('>BBHHHd', data_type, data_code, icmp_chesksum,
                              data_ID, data_Sequence, payload_body)
    return imcp_packet


def send_once(dst_addr, imcp_packet):
    '''
    连接套接字,并将数据发送到套接字
...
def reply_ping(rawsocket, timeout=1):
    '''
    接收icmp报文
    :param rawsocket:
    :param timeout:
...
def ping(host, count, timeout):
    ...
    for i in range(count):
        i += 1
        icmp_packet = request_ping(i)
        #print(icmp_packet)
        rowsocket, dst_addr = send_once(dst_addr, icmp_packet)
        ttl, sequence = reply_ping(rowsocket, timeout)
        if ttl < 0:
            print(f"{dst_addr}请求超时")
            lost += 1
            times.append(timeout * 1000)
        else:
            ttl = ttl * 1000
            print("来自 {0} 的回复: 字节=32 seq = {1} 时间={2:.2f}ms".format(
                dst_addr, sequence, ttl))
            accept += 1
            sumtime += ttl
            times.append(ttl)
    #统计
...
往返行程的估计时间(以毫秒为单位):最短 = {4:.2f}ms，最长 = {5:.2f}ms，平均 = {6:.2f}ms'.format(
        ...
        max(times),
        sum(times) // (lost + accept)))


if __name__ == "__main__":
    ping("10.2.248.249", 2, 20)