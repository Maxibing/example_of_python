...
class main(QMainWindow):
    ...
    def group_op(self):
        ...
        # 添加参数元素到参数框中
        self.group_op_layout.setWidget(0, QFormLayout.LabelRole, self.lab_src_addr)
        self.group_op_layout.setWidget(1, QFormLayout.LabelRole, self.le_src_addr)
        self.group_op_layout.setWidget(2, QFormLayout.LabelRole, self.lab_src_port)
        self.group_op_layout.setWidget(3, QFormLayout.LabelRole, self.le_src_port)
        self.group_op_layout.setWidget(4, QFormLayout.LabelRole, self.lab_dst_addr)
        self.group_op_layout.setWidget(5, QFormLayout.LabelRole, self.le_dst_addr)
        self.group_op_layout.setWidget(6, QFormLayout.LabelRole, self.lab_dst_port)
        self.group_op_layout.setWidget(7, QFormLayout.LabelRole, self.le_dst_port)
        self.group_op_layout.setWidget(8, QFormLayout.LabelRole, self.btn_listen)
        self.group_op_layout.setWidget(9, QFormLayout.LabelRole, self.btn_stop)
        self.group_op_layout.setWidget(10, QFormLayout.LabelRole, self.btn_clear)
    ...
    def mySingal_log(self, str):
        # LOG添加时间戳
        self.logWindow.append(time.strftime("%Y-%m-%d %H:%M:%S  ", time.localtime()) + str)
    ...
    def send(self):
        ...
        if not re.match(ipv6_regex, host):
            self._singal_log.emit("ERROR! Client address is invalid: Check address is right ipv6 address.")
            return 

        # 判断端口
        if port == "" or int(port) < 1 or int(port) > 65535:
            self._singal_log.emit("ERROR! Client Port is invalid: Check port in  1~65536.")
            return 
    ...
    def listen(self):
        ...
        ipv6_regex = r"[a-f0-9]{1,4}(:[a-f0-9]{1,4}){7}|[a-f0-9]{1,4}(:[a-f0-9]{1,4}){0,7}::[a-f0-9]{0,4}(:[a-f0-9]{1,4}){0,7}"
        if not re.match(ipv6_regex, host):
            self._singal_log.emit("ERROR! Server address is invalid: Check address is right ipv6 address.")
            return 

        # 判断端口
        if port == "" or int(port) < 1 or int(port) > 65535:
            self._singal_log.emit("ERROR! Server Port is invalid: Check port in  1~65536.")
            return

        # 创建监听服务
        global th_listen
        th_listen = threading.Thread(target=self.udpU6Server, daemon=True, args=(host, port))
        th_listen.start()
    ...
    def udpU6Server(self, host, port):
        ...
        try:
            ...
            self._singal_log.emit("ERROR! Start listening failure. Check address and port is available.")
            return
        ...
        while True:
            ...
            log = "Receive from: " + str(udpU6ServerInfo) + "  Data:" + str(udpU6Data_hex)
            self._singal_log.emit(log)
